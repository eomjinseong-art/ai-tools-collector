...
매일 GitHub Actions에서 실행되어, 20개 카테고리별로 YouTube에서 영상을 검색하고
Claude로 요약한 뒤 Supabase에 저장합니다.

## 동작 방식

1. Supabase `categories` 테이블에서 카테고리(20개)와 검색 키워드를 읽어옵니다.
...
5. 조회수와 게시일을 함께 고려한 점수(`view_count / (daysAgo+2)^1.4`)로 정렬해 카테고리당 상위 10개를 선정합니다.
6. 새로 추가되었거나 제목/설명이 바뀐 영상만 자막(가능한 경우)과 함께 Claude(`claude-fable-5`)로 요약합니다 — 이미 요약된 영상은 API 비용을 아끼기 위해 건너뜁니다.
7. 카테고리별로 상위 10개 밖으로 밀려난 기존 영상은 삭제하고, 최신 상위 10개를 upsert합니다.
...
2. `supabase/seed_categories.sql` — 20개 카테고리 삽입 (재실행해도 안전, slug 기준 upsert)

## YouTube API 쿼터 참고

- `search.list`는 호출당 100 유닛을 소모합니다. 카테고리 20개 × 키워드 약 3~4개 ≈ 검색 70~80회 ≈ **7,000~8,000 유닛/일**로, 기본 일일 쿼터(10,000 유닛)에 근접합니다.
